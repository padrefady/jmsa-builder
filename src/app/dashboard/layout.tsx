...
function WelcomeDialog() {
  const { user, welcomeShown, dismissWelcome } = useCMSStore();
...
function Header() {
  const { user, toggleSidebar, logout } = useCMSStore();
  const pathname = usePathname();

  const viewLabels: Record<string, string> = {
    '/dashboard': 'Tableau de bord',
    '/dashboard/sites': 'Sites web',
    '/dashboard/statistics': 'Statistiques',
    '/dashboard/settings': 'Parametres',
  };

  const currentLabel = viewLabels[pathname] || 'Tableau de bord';
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] text-gray-400 leading-tight hidden sm:block">{currentLabel}</span>
          </div>
        </div>
      </div>
...
export default function DashboardLayout({
  ...
  useEffect(() => {
    if (isAuthenticated && isPendingUser() && pathname !== '/dashboard' && pathname !== '/dashboard/settings') {
      router.replace('/dashboard');
    }
  }, [isAuthenticated, isPendingUser, pathname, router]);
